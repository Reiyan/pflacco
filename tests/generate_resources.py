...
def gen_misc_features():
    ...
    for fid in range(1, 25):
        for dim in DIMS:
            ...
            grad = calculate_gradient_features(
                f, dim, lower_bound=-5, upper_bound=5, seed=200, budget_factor_per_dim=10
            )
            ls = calculate_length_scales_features(
                f, dim, lower_bound=-5, upper_bound=5, seed=200, budget_factor_per_dim=10
            )
            si = calculate_sobol_indices_features(
                f, dim, lower_bound=-5, upper_bound=5, seed=200, sampling_coefficient=100
            )
    ...
    for fid in range(1, 25):
        for dim in DIMS:
            ...
            nodes, edges = compute_local_optima_network(
                f, dim, lower_bound=-5, upper_bound=5, seed=200, basin_hopping_iteration=10, stopping_threshold=100
            )
