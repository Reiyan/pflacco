...
def test_different_bounds_sample():
    ...
    expected = pd.read_pickle(os.path.join(RSC, "bound_sample.pkl"))
    assert sample.equals(expected)


def test_sobol_sample():
    sample = create_initial_sample(2, sample_type="sobol", seed=50)
    expected = pd.read_pickle(os.path.join(RSC, "sobol_sample.pkl"))
    assert sample.equals(expected)


def test_random_sample():
    sample = create_initial_sample(5, sample_type="random", seed=50)
    expected = pd.read_pickle(os.path.join(RSC, "random_sample.pkl"))
    assert sample.equals(expected)
...
def test_random_mixed_search_space_sample(x_mixed_search_space_sample):
    sample = create_initial_sample(
        3,
        sample_coefficient=3,
        sample_type="random",
        categorical_values=["cont", "int", ["val1", "val2", "val3"]],
        seed=50,
    )
