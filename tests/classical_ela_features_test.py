...
from .conftest import assert_features_equal
...
def test_calculate_ela_curvate(x_samples, feature_values):
    ...
    # factor of two, the quantiles by up to 9 percent, and even nas moves by one
    # point, because whether a point counts as degenerate is decided by
    # eig.min() > 0, exactly at the boundary. The grad_norm and grad_scale columns
    # of the same feature set agree to 1e-15 and stay in the comparison.
    unstable = r"ela_curv\.hessian_cond\."
    colnames = result.columns[~result.columns.str.contains("costs_runtime") & ~result.columns.str.match(unstable)]
    assert_features_equal(result[colnames], feature_values[colnames])
...
def test_calculate_cm_angle(x_samples, cm_feature_values):
    ...
    result = pd.concat(result).reset_index(drop=True)
    colnames = result.columns[~result.columns.str.contains("costs_runtime")]
    assert_features_equal(result[colnames], cm_feature_values[colnames])
...
def test_block_value_prerequisite(x_samples):
    with pytest.raises(
        ValueError, match='The provided value for "block" is too large, resulting in less than 3 observations per cell.'
    ):
        fid = 1
        dim = 10
        tmp = x_samples.iloc[: (dim * 50), :dim]
...
def test_block_value_too_low(x_samples):
    with pytest.raises(
        ValueError, match="The cell convexity features can only be computed when all dimensions have more than 2 cells."
    ):
        fid = 1
        dim = 2
        tmp = x_samples.iloc[: (dim * 50), :dim]
...
def test_block_value_forced(x_samples):
    with pytest.warns(
        UserWarning,
        match=r"For the given dataframe X, blocks \[.*\] require at least \d+ observations to retain 3 observations per cell. X only provides \d+.",
    ):
        fid = 1
        dim = 3
        tmp = x_samples.iloc[: (dim * 50), :dim]
...
def test_features_are_reproducible_from_a_seed(x_samples):
    ...
    calls = [
        lambda s: calculate_ela_conv(X, y, f, seed=s),
