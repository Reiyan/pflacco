...
def compute_local_optima_network(
    ...
    if minimizer_kwargs is None:
        minimizer_kwargs = {"method": "L-BFGS-B", "options": {"maxiter": 150, "ftol": 1e-7}}
    ...
    for _ in range(basin_hopping_iteration):
        ...
        opt.basinhopping(
            ...
            stepsize=stepsize,
            callback=_minfound,
            niter=stopping_threshold,
        )
        restart = True

    nodes = pd.DataFrame(
        np.array([np.array([x, nodes[x][0], 1]) for x in range(len(nodes))]), columns=["id", "fval", "neutrality"]
    )
...
def calculate_lon_features(
    nodes: pd.DataFrame, edges: pd.DataFrame, f_opt: float = None
