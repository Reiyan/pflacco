...
from .sampling import _create_local_search_sample
from .sampling import _levy_random_walk
...
def calculate_hill_climbing_features(
    ...
    opt_result, nfvals = _create_local_search_sample(
        f,
        dim,
        lower_bound,
        upper_bound,
        n_runs=n_runs,
        budget_factor_per_run=budget_factor_per_run,
        method=method,
        minimize=minimize,
        seed=seed,
    )
...
def calculate_fitness_distance_correlation(
    ...
    if proportion_of_best < 1:
        ...
        if round(len(sorted_idx) * proportion_of_best) < 2:
            raise Exception(
                f"Selecting only {proportion_of_best} of the sample results in less than 2 remaining observations."
            )
...
def calculate_sobol_indices_features(
    ...
    pdef = {"num_vars": dim, "names": X.columns, "bounds": list(zip(lower_bound, upper_bound))}
