...
import numpy as np
import pandas as pd
from numdifftools.core import Gradient
...
from .utils import _cartesian_product_efficient
from .utils import _check_blocks_variable
from .utils import _create_blocks
from .utils import _normalize_objective
from .utils import _normalize_objective_with_scale
from .utils import _transform_bounds_to_canonical
from .utils import _validate_variable_types
...
def calculate_nbc(
    ...
    if fast_k < 0 or fast_k > X.shape[0]:
        raise ValueError(
            f'[{fast_k}] of "fast_k" does not lie in the interval [0,n] where n is the number of observations.'
        )
    if minimize == False:
        y = y * -1
...
def calculate_dispersion(
    ...
    keys = [f"disp.ratio_mean_{int(round(x * 100, 0)):0>2d}" for x in disp_quantiles]
    keys.extend([f"disp.ratio_median_{int(round(x * 100, 0)):0>2d}" for x in disp_quantiles])
    keys.extend([f"disp.diff_mean_{int(round(x * 100, 0)):0>2d}" for x in disp_quantiles])
    keys.extend([f"disp.diff_median_{int(round(x * 100, 0)):0>2d}" for x in disp_quantiles])
...
def calculate_information_content(
    ...
    if ic_sorting == "random":
        ...
        if ic_nn_neighborhood < 1 and ic_nn_neighborhood > X.shape[0]:
            raise Exception(
                f"[{ic_nn_neighborhood}] is an invalid option for the NN neighborhood, because the sample only covers 1 to {X.shape[0]} observations."
            )
        ...
        for i in range(1, X.shape[0]):
            ...
            if len(current) > 0:
                ...
                (
                    distances2,
                    indices2,
                ) = nbrs2.kneighbors(X.iloc[permutation[i - 1]].to_numpy().reshape(1, X.shape[1]))
...
def calculate_limo(
    X: pd.DataFrame | np.ndarray | list[list[float]],
    y: pd.Series | np.ndarray | list[float],
    lower_bound: list[float] | float,
    upper_bound: list[float] | float,
    blocks: list[int] | np.ndarray | int | None = None,
    force: bool = False,
    normalize: bool = True,
    ...
    coeff_ratio = np.array(
        [np.max(np.abs(coeff)) / np.min(np.abs(coeff)) if ~np.isnan(coeff).all() else None for coeff in coeff_vector]
    )
    ...
    if len(blocks) > 1:
        ...
        result["limo.cor"] = (np.sum(cor_unscaled) - np.sum(np.diag(cor_unscaled))) / (
            np.ma.count(cor_unscaled) - len(np.diag(cor_unscaled))
        )
        result["limo.cor_norm"] = (np.sum(cor_scaled) - np.sum(np.diag(cor_scaled))) / (
            np.ma.count(cor_scaled) - len(np.diag(cor_scaled))
        )
...
def calculate_cm_conv(
    ...
    if all_centers.any():
        ...
        nbrs_backup = NearestNeighbors(n_neighbors=n_ctr + 1, algorithm="kd_tree").fit(
            np.vstack([cell_centers[all_centers], X.to_numpy()])
        )
...
def calculate_ela_level(
    ...
    ela_level_resample_iterations: int = 10,
) -> dict[str, int | float]:
    ...
    for prob in ela_level_quantiles:
        ...
        if y_class.sum() < ela_level_resample_iterations:
            raise Exception(
                f"There are too few observation in case of quantile {prob} to perform resampling with {ela_level_resample_iterations} folds."
            )
    ...
    for i in range(len(ela_level_quantiles)):
        name = f"ela_level.mmce_lda_{ela_level_quantiles[i] * 100:.0f}"
        result[name] = lda_mmce[i]

        name = f"ela_level.mmce_qda_{ela_level_quantiles[i] * 100:.0f}"
        result[name] = qda_mmce[i]

        name = f"ela_level.lda_qda_{ela_level_quantiles[i] * 100:.0f}"
        result[name] = lda_qda[i]

        if interface_mda_from_R:
            name = f"ela_level.mmce_mda_{ela_level_quantiles[i] * 100:.0f}"
            result[name] = mda_mmce[i]

            name = f"ela_level.lda_mda_{ela_level_quantiles[i] * 100:.0f}"
            result[name] = lda_mda[i]

            name = f"ela_level.qda_mda_{ela_level_quantiles[i] * 100:.0f}"
...
def calculate_ela_local(
    ...
    **minimizer_kwargs,
    ...
    if X.shape[0] < N:
        raise ValueError(
            f"X contains less then the required {N} (= dim * ela_local_local_searches_factor) starting points"
        )
    rng = np.random.default_rng(seed)
    ...
    return {
        ...
        "ela_local.best2mean_contr.ratio": (center_fvals.mean() - center_fvals.min())
        / (center_fvals.max() - center_fvals.min()),
        "ela_local.basin_sizes.avg_best": clust_sizes[center_best_idx].mean(),
        "ela_local.basin_sizes.avg_non_best": 0
        if len(clust_sizes[~center_best_idx]) == 0
        else clust_sizes[~center_best_idx].mean(),
